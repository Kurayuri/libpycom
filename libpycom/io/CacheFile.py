...
class CacheFile:
    ...
    def __init__(self, path, 
                 filename_pattern: str | EllipsisType = ..., 
                 fn_cache: int | Callable | None = None, 
                 fn_load: Callable | None = None,
                 dtype: Any = None
                 ):
    ...
    def load(self):
        ...
        import pandas as pd
        if self.check():
            self.messager.info("Load from Cache")

            if self.dtype is pd.DataFrame:
                self._load_data = self.load_dataframe(self.cache_path)
        
        else:
            self.messager.info("Load from Source File")
            self._load_data = self.fn_load(self.path) if callable(self.fn_load) else None
            self.messager.info("Cache Data")
            if self.dtype is pd.DataFrame:
                self.cache_dataframe(self.cache_path, self._load_data)
